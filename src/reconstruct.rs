...
pub fn reconstruct_slice_no_deblock<R: RefPicProvider>(
    ...
    // !field_pic_flag. Field-only pictures (`field_pic_flag == 1`) are
    // PAFF (Picture Adaptive Frame/Field); each picture is one field
    // at half the frame height. MBs are walked in raster order (non-MBAFF path).
    let mbaff_frame_flag = sps.mb_adaptive_frame_field_flag && !slice_header.field_pic_flag;
